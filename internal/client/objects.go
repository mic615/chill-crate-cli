...
func (c *Client) UploadObject(bucketID, fileName string, body io.Reader, size int64) (Object, error) {
	var object Object
	err := c.doReader("POST", fmt.Sprintf("/buckets/%s/objects/%s", bucketID, fileName), body, size, &object)
	return object, err
}
